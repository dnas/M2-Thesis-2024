...
const GameType kGameType{/*short_name=*/"preflop",
                         /*long_name=*/"Preflop Pot Limit Omaha",
...
class PreflopObserver : public Observer {
 public:
  ...
  static void WriteSinglePlayerBucket(const PreflopState& state, int player,
                                    Allocator* allocator) {
  ...
  static void WriteAllPlayerBuckets(const PreflopState& state,
                                  Allocator* allocator) {
...
PreflopState::PreflopState(std::shared_ptr<const Game> game)
  ...
  num_buckets_.resize(game->NumPlayers());
  transition_matrix_.resize(game->NumPlayers());
  preflop_prob_.resize(game->NumPlayers());
  for(int i=0;i<game->NumPlayers();i++) num_buckets_[i] = kDefaultBuckets[i];
  for(int i=0;i<game->NumPlayers();i++) preflop_prob_[i].resize(num_buckets_[i]);
  for(int i=0;i<game->NumPlayers();i++){
    std::ifstream ifile;
    std::string file_name = "preflop_buckets_prob_"+std::to_string(num_buckets_[i])+".txt";
    ifile.open(file_name);
    for(int j=0;j<num_buckets_[i];j++) ifile >> preflop_prob_[i][j];
    ifile.close();
    transition_matrix_[i].resize(num_buckets_[i]);
    file_name = "postflop_buckets_matrix_"+std::to_string(num_buckets_[i])+".txt";
    ifile.open(file_name);
    for(int j=0;j<num_buckets_[i];j++){
      transition_matrix_[i][j].resize(num_buckets_[i]);
      for(int k=0;k<num_buckets_[i];k++) ifile >> transition_matrix_[i][j][k];
    }
    ifile.close();
  }
}
...
void PreflopState::DoApplyAction(Action move) {
  ...
  if (IsChanceNode()) {
    ...
    int dealing_to = round_==0?private_bucket_pre_dealt_:private_bucket_post_dealt_;
    SPIEL_CHECK_LT(move, num_buckets_[dealing_to]);

    if (round_==0&&private_bucket_pre_dealt_ < num_players_) { //round 0 - preflop
...
std::vector<Action> PreflopState::LegalActions() const {
  ...
  if (IsChanceNode()) {
    int dealing_to = round_==0?private_bucket_pre_dealt_:private_bucket_post_dealt_;
    movelist.resize(num_buckets_[dealing_to]);
...
std::vector<std::pair<Action, double>> PreflopState::ChanceOutcomes() const {
  ...
  if(round_==0){ //deal buckets to each player. We're dealing private_bucket_pre_dealt_
    for(int i=0;i<num_buckets_[private_bucket_pre_dealt_];i++) outcomes.push_back({i, preflop_prob_[private_bucket_pre_dealt_][i]});
  }else if(round_==1){ //the current player we're dealing with is private_bucket_post_dealt_. We retrieve its preflop bucket, and return the corresponding row in the transition matrix
    for(int i=0;i<num_buckets_[private_bucket_post_dealt_];i++) outcomes.push_back({i, transition_matrix_[private_bucket_post_dealt_][private_bucket_pre_[private_bucket_post_dealt_]][i]});
  }else{
...
void PreflopState::ResolveWinner() {
  ...
  if ((int)players_remaining_.size() == 1) {
    ...
    std::vector<double> eqsq(num_players_, 0.0);
    double sum_eqsq = 0;
    for(Player p:players_remaining_){
      double bucket_len = 1.0/(double)num_buckets_[p];
      eqsq[p] = (bucket_len*(double)private_bucket_post_[p]+bucket_len/2)*(bucket_len*(double)private_bucket_post_[p]+bucket_len/2);
      sum_eqsq += eqsq[p];
    }
    int tot = 0;
    for(Player p:players_remaining_){
      int removed = static_cast<double>(pot_) * eqsq[p]/sum_eqsq;
      stack_[p] += removed;
      tot += removed;
    }
    pot_ -= tot;
...
void PreflopState::SetPrivateBuckets(const std::vector<int>& new_private_bucket) {
