...
inline const std::vector<int> kDefaultBuckets = {10, 1};
...
class PreflopState : public State {
 ...
 private:
  ...
  int private_bucket_pre_dealt_;  // How many private buckets currently dealt preflop.
  ...
  //The probability of getting dealt each bucket preflop, read from "preflop_buckets_prob.txt"
  std::vector<std::vector<double>> preflop_prob_;
  //The n*n probability transition matrix from preflop to flop, read from "preflop_buckets_prob.txt"
  std::vector<std::vector<std::vector<double>>> transition_matrix_;
  ...
  std::vector<int> num_buckets_; //how many buckets to group hands into
};
...
class PreflopGame : public Game {
 ...
 private:
  ...
  std::vector<int> num_buckets_; //Number of buckets in the abstraction
  // Players cannot distinguish between cards of different suits with the same
  // rank.
